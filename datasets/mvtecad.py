...
class MVTecAD(BaseADDataset):
    ...
    def __init__(self, args, train = True):
        ...
        #设置数据集根目录、选择训练或测试的数据变换（transform），以及伪标签（pseudo label）的数据变换
        ...
        #初始化一个空列表normal_data，设置数据集划分（train/test）
        normal_data = list()
        split = 'train'
        ...
        #调用get_ood_data方法获取OOD（Out-of-Distribution）数据
        self.ood_data = self.get_ood_data()
        #如果是测试集，重新初始化normal_data，遍历测试集中的正常类别
...
#接下来是一系列方法的定义，包括获取OOD数据、划分异常数据、加载图像、定义数据变换等。这些方法主要用于数据的预处理和加载
