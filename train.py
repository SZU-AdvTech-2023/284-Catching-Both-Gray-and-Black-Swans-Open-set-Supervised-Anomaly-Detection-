...
#生成目标标签，根据是否在评估阶段选择不同的处理方式
    def generate_target(self, target, eval=False):
        ...
        if eval:
            ...
            targets.append(target != 0)
        return targets
#训练模型的方法，包括模型前向传播、损失计算、梯度反向传播和优化器更新
...
#用于数据归一化的方法
    def normalization(self, data):
        return data
#评估模型性能的方法，包括模型前向传播、计算损失、计算性能指标（AUC-ROC和AUC-PR）、保存评估结果
    def eval(self):
        self.model.eval()
...
def aucPerformance(mse, labels, prt=True):
    ...
    return roc_auc, ap;
#脚本的主程序入口，通过解析命令行参数初始化训练器（Trainer）对象
if __name__ == '__main__':
    ...
    parser.add_argument('--savename', type=str, default='model.pth', help="save modeling")
    parser.add_argument('--dataset_root', type=str, default='./data/mvtec_anomaly_detection', help="dataset root")
    parser.add_argument('--experiment_dir', type=str, default='./experiment-10-30-10-90/experiment_14', help="dataset root")
    ...
    parser.add_argument("--nAnomaly", type=int, default=1, help="the number of anomaly data in training set")
...
#根据命令行参数配置是否使用GPU，然后创建Trainer对象
    args.cuda = not args.no_cuda and torch.cuda.is_available()
    trainer = Trainer(args)

#保存实验的设置信息，包括命令行参数和其对应的数值
...
#输出总的训练轮数，并将模型和损失函数移至GPU，然后开始进行模型的训练
